...
//	文件名称	:  sc32f12xx_TK_FileAnalysis.c
//	作者		: 
//	模块功能	:  触控键配置文件
// 	版本		:
// 	更改记录	:
...
//  库版本标记	: 
//************************************************************************************************
#include "sc32f12xx_TK_ParameterAnalysis.h"
...
uint32_t	CombineCurrentChannelNum1; 
uint32_t	CombineCurrentChannelNum2; 
uint32_t	CombineCurrentChannelNum3; 
uint32_t	CombineCurrentChannelNum4; 
uint32_t	CombineCurrentChannelNum5; 
uint32_t	CombineCurrentChannelNum6; 
...
//===============================================================================
//自定义变量
uint16_t 	UpdateBaseLNum; 	    
uint16_t 	MultipleLNum;			
...
uint64_t TK_TouchKeyScan(void)
{
	...
	if(TK_GetIsNeedUpdateBaseline() == 0)				//检测是否需要更新baseline 
	{
		...
		{
			if(((uint64_t)0x01 << t) & Keyvalue)
			{
				KeyData |= ((uint64_t)0x01 << (CurrentChannel[t]));              
				MultipleCnt++;							
			}
		}                
